...
flags.DEFINE_string("pre_trained_word2vec", "result_pubmed_mesh.txt", "The vocabulary file that the BioWordVec model was trained on.")
...
# COMMON FLAGS
flags.DEFINE_string("workspace_dp", '/nfsvol2/projects/umls/deeplearning/aaai2020/2019AB', "The output directory where the model checkpoints will be written.")
flags.DEFINE_string("umls_dp", '/nfsvol2/projects/umls/deeplearning/aaai2020/2019AB/UMLS_VERSIONS/2020AA-ACTIVE', "The output directory where the model checkpoints will be written.")
...
flags.DEFINE_string('umls_dl_dp', '/nfsvol2/projects/umls/deeplearning/aaai2020/2019AB/UMLS_VERSIONS/2020AA-ACTIVE/META_DL', '')
...
# flags.DEFINE_string("train_dataset_dp", '/nfsvol2/projects/umls/deeplearning/aaai2020/2019AB/UMLS_VERSIONS/2020AA-ACTIVE/TRAINSET', "")
# flags.DEFINE_string("train_dataset_dp", '/nfsvol2/projects/umls/deeplearning/aaai2020/2019AB/UMLS_VERSIONS/2020AA-ACTIVE/2020AA-ACTIVE_MODEL_TRAINING_DS_TOPN_SIM', "")
# flags.DEFINE_string("test_dataset_dp", '/nfsvol2/projects/umls/deeplearning/aaai2020/2019AB/UMLS_VERSIONS/2020AA-ACTIVE/2020AA-ACTIVE_MODEL_GENTEST_DS', ""
# flags.DEFINE_string("test_dataset_dp", '/nfsvol2/projects/umls/deeplearning/aaai2020/2019AB/UMLS_VERSIONS/2020AA-ACTIVE/TESTSET', '')
...
flags.DEFINE_string("embedding_fp", '/data/Bodenreider_UMLS_DL/extra/biowordvec.txt', "Embedding dictionary pickle file")

#flags.DEFINE_string("aui2convec_pickle_fp", None, "The location of the pickle file for aui2convec")
...
flags.DEFINE_string("word_embedding", "BioWordVec", "Word embedding layer")
...
flags.DEFINE_integer("context_vector_dim", 200, "Dimension for the context vector")
...
flags.DEFINE_list('metric_names', ['epoch', 'accuracy', 'precision', 'recall', 'f1', 'auc'], '')
flags.DEFINE_list('metric_labels', ['Epoch', 'Accuracy', 'Precision', 'Recall', 'F1', 'AUC'], '')
...
flags.DEFINE_string('KGE_Home', '/data/Bodenreider_UMLS_DL/ISWC2021/KGE', 'Path to the home directory of KGE')
...
class DataGenerator(tf.keras.utils.Sequence):
    ...
    def __read_batch_from_indexes(self, list_IDs_temp):
        ...
        lefts = np.empty((batch_size, self.max_seq_length), dtype = float)
        rights = np.empty((batch_size, self.max_seq_length), dtype = float)
        labels = None
        if not self.is_test:
...
def predict_generator(generator, labels, labels_size,batch_size, model=None, epoch = None, log_scores = None, raw=False):
    ...
    # mandist = ManDist()([left_base_input, right_base_input])
    ...
    predictions = [x[0] for x in predict]
    #predictions = [1 for x in range(8900000)]
    #logger.debug("predictions: {}".format(predictions))
    #logger.debug("predict: {}".format(predict))
    #test_list(predictions, 'test predictions')
    #y_pred = np.array(prediction).reshape(y_true.shape)
    ...
    return predictions if raw else y_pred
...
def save_metrics(dp, metrics_fn, test_metrics):
    ...
    if test_metrics is not None:
        ...
        for test_fn in test_metrics.keys():
            ...
            with open(os.path.join(dp,'%s_%s'%(name, metrics_fn)), 'w') as f:
                s = FLAGS.delimiter
                f.write(name)
                f.write(FLAGS.delimiter.join(metrics))
                f.write("\n")
...
def gen_aui2vec(tokenizer, aui2vec_pickle_fp, mrc_atoms):
    aui2vec = dict()
    #logger.info("Loading aui list from %s ..."%mrconso_master_fp)
    auis_str = get_auis_str(mrc_atoms)
...
def gen_bert_aui2vec(vocab_file, aui2vec_pickle_fp, mrconso_master_fp):
    ...
    with tqdm(total = len(auis_str)) as pbar:
        for aui_id, aui_str in auis_str.items():
            if aui_str !=  '':
                #Convert word to sequence and padding sequence.
                encoded_aui = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(aui_str))
                if aui_id < 2:
                    logger.debug('tokenizing id = {}, aui = {} to {}'.format(aui_id, aui_str, encoded_aui))
                padded_aui = pad_sequences([encoded_aui], maxlen = FLAGS.max_seq_length,
                                            padding = FLAGS.padding_sequence,
...
def get_auis_str(mrc_atoms):
        aui_strs = dict()
        for aui, row in mrc_atoms.items():
            aui_strs[row['ID']] = row['STR']
        return aui_strs
...
def create_model(embedding_matrix):
    ...
    if FLAGS.exp_flavor == 1:
        ...
        if FLAGS.use_shared_dropout_layer:
            ...
            malstm_distance = ManDist()([x5_left, x5_right])

        else:
            x3_left, x3_right = shared_dense_2(x2_left), shared_dense_2(x2_right)
            malstm_distance = ManDist()([x3_left, x3_right])
        
        
        model = Model(inputs = [left_base_input, right_base_input], outputs = [malstm_distance])
        ...
        model = Model(inputs = [left_base_input, left_context_input, right_base_input, right_context_input], outputs = [malstm_distance])
        ...
        model = model_exp.make_model(malstm_distance, left_base_input, right_base_input, left_enrich_input, right_enrich_input)
...
def main(_):
    ...
    with tf.compat.v1.Session(config = config_proto) as sess2:
        ...
        aui2convec_pickle_fp = os.path.join(FLAGS.KGE_Home, FLAGS.Embeddings, FLAGS.Model, FLAGS.ConVariant, FLAGS.ConVariant + '_' + FLAGS.AUI2CONVEC)
