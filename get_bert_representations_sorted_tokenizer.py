...
if not(os.path.exists('/data/Bodenreider_UMLS_DL/Interns/Bernal/{}_vecs'.format(summary_model_name))):
    os.makedirs('/data/Bodenreider_UMLS_DL/Interns/Bernal/{}_vecs'.format(summary_model_name))
...
with torch.no_grad():
    ...
    for i,row in tqdm(sorted_umls_df.iterrows(),total=len(sorted_umls_df)):
        ...
        if pad_size * len(text_batch) > 6000 or num_strings_proc == len(sorted_umls_df):
            ...
            if not(os.path.exists('/data/Bodenreider_UMLS_DL/Interns/Bernal/{}_vecs/umls2020_{}_vecs_{}.p'.format(summary_model_name,summary_model_name,vec_save_batch_num))):
            ...
            if curr_vecs == 100:
                ...
                if not(os.path.exists('/data/Bodenreider_UMLS_DL/Interns/Bernal/{}_vecs/umls2020_{}_vecs_{}.p'.format(summary_model_name,summary_model_name,vec_save_batch_num))):
                    ...
                    pickle.dump(all_cls, open('/data/Bodenreider_UMLS_DL/Interns/Bernal/{}_vecs/umls2020_{}_vecs_{}.p'.format(summary_model_name,summary_model_name,vec_save_batch_num),'wb'))
